...
def _get_project_user_for_document(session: SessionDep, user_id: UUID, project_id: UUID, document_id: UUID) -> Tuple[Document, ProjectUser]:
...
def validate_permissions_for_document(session: SessionDep, user_id: UUID, project_id: UUID, document_id: UUID) -> Tuple[Document, Role]:
    document, project_user = _get_project_user_for_document(session, user_id, project_id, document_id)
...
def get_document_for_user_permissions(project_id: Annotated[UUID, Path()], document_id: Annotated[UUID, Path()], session_and_user: Tuple[User, SessionDep] = Depends(get_user_and_session)) -> Tuple[Document, SessionDep]:
    current_user, session = session_and_user # Validate user's credentials in get_user_and_session()
    document, _role = validate_permissions_for_document(session, current_user.id, project_id, document_id) # Validate permissions

    return document, session


def get_document_for_owner_permissions(project_id: Annotated[UUID, Path()], document_id: Annotated[UUID, Path()], session_and_user: Tuple[User, SessionDep] = Depends(get_user_and_session)) -> Tuple[Document, SessionDep]:
    current_user, session = session_and_user # Validate user's credentials in get_user_and_session()

    document, role = validate_permissions_for_document(session, current_user.id, project_id, document_id) # Validate permissions
    # Only project owner can perform actions
    if role != Role.OWNER:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions.")
