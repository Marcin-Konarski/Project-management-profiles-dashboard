...
class TestCreateProjects:
    ...
    def test_create_project_returns_201(self, authenticated_client: TestClient, mock_session: MagicMock, fake_user: MagicMock, fake_project_payload):
    ...
    def test_create_project_with_document_returns_201(self, authenticated_client: TestClient, mock_session: MagicMock, fake_user: MagicMock, fake_project_with_documents_payload):
        response = authenticated_client.post('/projects/', json=fake_project_with_documents_payload)

        assert response.status_code == status.HTTP_201_CREATED
        assert response.json()["name"] == fake_project_with_documents_payload.get("name")
        assert response.json()["description"] == fake_project_with_documents_payload.get("description")
        ...
        assert first_call_arg.description == fake_project_with_documents_payload["description"]
        assert first_call_arg.owner_id == fake_user.id
        ...
        assert len(first_call_arg.documents) == len(fake_project_with_documents_payload["documents"]) # Verify that i has correct number of elements
        assert all(isinstance(doc, Document) for doc in first_call_arg.documents) # Verify that all documents are of correct type
        ...
        for doc, expected in zip(first_call_arg.documents, fake_project_with_documents_payload["documents"]):
            assert doc.project_id == first_call_arg.id # IDs must match (That is the place where I found first actual bug :>)
            assert doc.name == expected["name"]
            assert doc.storage_key == expected["storage_key"]
            assert doc.size == expected["size"]


    def test_create_project_but_user_not_auth_returns_401(self, client: TestClient, mock_session: MagicMock, fake_project_payload):
    ...
    def test_create_project_but_project_name_already_exists_returns_409_(self, authenticated_client: TestClient, mock_session: MagicMock, fake_project_payload):
        ...
        assert response.json()["detail"] == f"Project with name '{fake_project_payload.get("name")}' already exists."
        mock_session.commit.assert_called_once()
        mock_session.rollback.assert_called_once()
        mock_session.refresh.assert_not_called()


    def test_create_project_but_document_name_already_exists_returns_409(self, authenticated_client: TestClient, mock_session: MagicMock, fake_project_with_documents_payload):
        document_name = fake_project_with_documents_payload["documents"][0].get("name") # Get the name of the document
        fake_unique_violation = MagicMock(spec=UniqueViolation)
        ...
        assert response.status_code == status.HTTP_409_CONFLICT
        assert response.json()["detail"] == f"Document with name '{fake_project_with_documents_payload['documents'][0].get("name")}' already exists."
        mock_session.commit.assert_called_once()
        mock_session.rollback.assert_called_once()
        mock_session.refresh.assert_not_called()
...
class TestListAllProjects:
    ...
    def test_list_all_projects_returns_200(self, authenticated_client: TestClient, mock_session: MagicMock, fake_user, fake_project_payload):
        project_object = Project(name=fake_project_payload.get("name"), description=fake_project_payload.get("description"), owner_id=fake_user.id)
    ...
    def test_list_all_projects_returns_200_with_empty_list(self, authenticated_client: TestClient, mock_session: MagicMock):
    ...
    def test_list_all_projects_returns_401_when_not_authenticated(self, client: TestClient, mock_session: MagicMock):
...
class TestGetProjectDetails:
    ...
    def test_get_project_details_returns_200(self, authenticated_client: TestClient, mock_session: MagicMock, fake_user: MagicMock, fake_project: Project):
        ...
        fake_user_2 = MagicMock() # Create second fake user that has access to the project
...
class TestUpdateProjectDetails:
    ...
    def test_update_project_name_only_returns_200(self, authenticated_client: TestClient, mock_session: MagicMock, fake_project: Project):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["name"] == update_body["name"]
        assert response.json()["description"] == original_description # Description must be unchanged
    ...
    def test_update_project_details_returns_409_when_name_already_exists(self, authenticated_client: TestClient, mock_session: MagicMock, fake_project: Project):
        ...
        update_body = {"name": f"{fake_project.name}", "description": "Test Description"}
        response = authenticated_client.put(f"/project/{fake_project.id}/info", json=update_body)

        assert response.status_code == status.HTTP_409_CONFLICT
        assert response.json()["detail"] == f"Project with name '{fake_project.name}' already exists."
        mock_session.commit.assert_called_once()
        mock_session.rollback.assert_called_once()
        mock_session.refresh.assert_not_called() # Must not refresh after failed commit


    def test_update_project_details_returns_401_when_not_authenticated(self, client: TestClient, mock_session: MagicMock, fake_project: Project):
        update_body = {"name": "Random Name", "description": "Random Description"} # Here body doesn't matter. It only has to pass pydantic validation
...
class TestUploadDocuments:
    ...
    def test_upload_documents_returns_201(self, authenticated_client: TestClient, mock_session: MagicMock, fake_project: Project):
        ...
        assert all(doc.project_id == fake_project.id for doc in added_documents)  # Verify that all are linked to correct project
    ...
    def test_upload_document_returns_409_when_document_name_already_exists(self, authenticated_client: TestClient, mock_session: MagicMock, fake_project: Project):
        ...
        mock_session.commit.side_effect = IntegrityError("duplicate", {}, fake_unique_violation) # Set up session.commit() to return IntegrityError with origin of UniqueViolation from PostgreSQL

        body = [{"name": "doc1", "storage_key": "key", "size": 5}]
        response = authenticated_client.post(f"/project/{fake_project.id}/documents", json=body)

        assert response.status_code == status.HTTP_409_CONFLICT
        assert response.json()["detail"] == f"Document with name '{body[0].get("name")}' already exists."
    ...
    def test_upload_document_returns_401_when_not_authenticated(self, client: TestClient, mock_session: MagicMock):
        body = [{"name": "doc1", "storage_key": "key", "size": 5}] # Again body doesn't really matter but must pass pydantic validation
...
class TestUpdateDocument:
    ...
    def test_update_document_returns_401_when_user_not_authenticated(self, client: TestClient, mock_session: MagicMock, fake_document: Document, fake_document_1_payload):
...
class TestDeleteDocument:
    ...
    def test_delete_document_returns_401_when_user_not_authenticated(self, client: TestClient, mock_session: MagicMock, fake_document: Document):
...
class TestAddUserToProject:
    ...
    def test_add_user_to_project_returns_401_when_not_authenticated(self, client: TestClient, mock_session: MagicMock, fake_project: Project):
